...
* Collections MUST only use the directories specified below in the ``plugins/`` directory and
  ...
  :sub_plugins: For other plugins which are managed by modules inside of collections instead of ansible-core.  We use a subfolder so there aren't conflicts when ansible-core adds new plugin types.
